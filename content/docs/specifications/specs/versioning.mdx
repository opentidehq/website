...
- [metadata.md](/docs/specifications/specs/metadata/) — defines `metadata.schema` and `metadata.version` fields
- [workspace.md](/docs/specifications/specs/workspace/) — paths for schemas, templates, and object directories
- Object specs under `objects/` — one spec per active schema revision

## Defaults & overrides

Path defaults are configured in bundled `paths.toml`; clients MAY override via [configuration.md](/docs/specifications/specs/configuration/). Schema artifacts under `.opentide/schemas/` MUST NOT be hand-edited.
