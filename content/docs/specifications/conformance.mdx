...
An object **conforms** to a spec when it satisfies every MUST and MUST NOT in that spec and in the shared [metadata](/docs/specifications/specs/metadata/) contract. The [validation pipeline](/docs/specifications/specs/validation/) enforces conformance mechanically:
...
A conforming **implementation** (such as opentide) validates objects exactly as the specs require and reports capabilities honestly — for example, never reporting query validation as passed for a platform that cannot perform it (see [validation](/docs/specifications/specs/validation/) and [platforms](/docs/specifications/specs/platforms/)).

## Authority

Where a spec and any generated artifact disagree, the **spec wins**. Generated JSON Schema exists for editor and router validation; it is derived from the specs, not the other way around. The precedence chain and change process are defined in [Governance](/docs/specifications/GOVERNANCE/).

## Versioning of conformance

There is no framework-wide version to conform to. An object conforms to the specific **schema revision** it declares in `metadata.schema` (e.g. `rule::1.0`), and each spec is versioned independently. See [Versioning](/docs/specifications/specs/versioning/).
