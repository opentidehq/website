...
With `--json`, failures still write one complete object to stdout before exiting. In the [tutorial](/docs/usage/tutorial/) repository with the dangling `detection_model` from [step 7](/docs/usage/tutorial/#7-break-it-on-purpose), `opentide --json validate` exits `1` and prints the whole report, trimmed here to its outcome:

```json output-of="opentide --json validate" exit=1 state=dangling-reference
{
...
```bash
if opentide --json validate > result.json; then
...
- [`validate`](/docs/cli/validate/) — `--strict` and object/query checks.
- [`deploy`](/docs/cli/deploy/) — deployment errors and warnings.
- [CI/CD](/docs/usage/workflows/ci-cd/) — pipelines that gate on these codes.
