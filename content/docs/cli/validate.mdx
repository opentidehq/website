...
Run from anywhere inside the detection workspace: the CLI walks up to it (see [Repository root resolution](/docs/cli/global-options/#repository-root-resolution)). The JSON payload names the directory it read objects from under `workspace`.
...
and a `findings` list. If the [tutorial](/docs/usage/tutorial/) rule's query
