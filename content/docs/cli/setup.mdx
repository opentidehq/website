...
VS Code and Cursor expand `${workspaceFolder}`. Claude Code expands `${CLAUDE_PROJECT_DIR}`. Generic hosts that do not interpolate editor placeholders should omit the variable so the server discovers the git root from cwd, or set an absolute path. See [MCP configuration](/docs/mcp/configuration/).
