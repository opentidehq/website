...
For agent/MCP hosts, remember the path to this environment's `opentide-mcp` binary — you point the host at it in [MCP configuration](/docs/mcp/configuration/).

## PyPI install

The latest release is **0.2.1** (18 September 2026). The first public release was **0.1.0** (9 September 2026). Notes: [Releases](/docs/usage/releases/).

```bash
pip install opentide              # latest on PyPI
pip install 'opentide==0.2.1'     # pin the current release
```
