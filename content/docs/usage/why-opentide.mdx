...
- **Explicit chaining.** Rules reference the objectives they satisfy; objectives reference the threats they cover. Coverage and gaps become queryable facts, not tribal knowledge. See the [object model](/docs/usage/concepts/object-model/).
- **One source, many platforms.** A rule carries per-platform configuration blocks. opentide deploys to each platform and, where the platform allows it, validates query syntax — [honestly reporting](/docs/usage/concepts/platforms/) where it cannot.
...
- **Automation as a first-class citizen.** The same engine is exposed as a [CLI](/docs/cli/), a Python [SDK](/docs/sdk/), and an [MCP server](/docs/mcp/) for agents — with capability reporting that never fakes a result.
...
| **Honest capability reporting** | If a platform cannot validate query syntax, the CLI and MCP say `supported: false` rather than returning a fake pass. See [Platforms](/docs/usage/concepts/platforms/). |
...
| **Git is the history** | Instance content versions (`metadata.version`) track meaning; git tracks the actual change history. See [Schema revision](/docs/usage/concepts/schema-revision/). |
...
- You want live query execution against every platform today — some platforms are deploy-only, and a couple of MCP query tools are still [stubs](/docs/usage/workflows/agentic-setup/). Check the [platform matrix](/docs/usage/concepts/platforms/) before committing.

## Next

- [How opentide works](/docs/usage/how-it-works/) — the end-to-end lifecycle.
- [Object model](/docs/usage/concepts/object-model/) — the three object families and chaining.
- [Installation](/docs/usage/installation/) — get the CLI on your machine. Latest release: [0.3.0](/docs/usage/releases/).
