...
See [Platforms](/docs/usage/concepts/platforms/) for the capability matrix and the exact `--platform` identifiers.
...
<Callout type="info">
Exact connection keys differ per platform. Use `opentide info --platform <name>` to see what a platform expects, and never commit real secrets — see [CI/CD](/docs/usage/workflows/ci-cd/#secrets) for injecting them in pipelines.
</Callout>
...
`opentide setup --no-promotion` or `--promotion-target <STATUS>` writes this table, with or without `--ci`; see [setup](/docs/cli/setup/#default-callback-flags).

There is no separate promote command. Edit a rule's `status` in YAML for deliberate changes, then run `opentide deploy` — when promotion is enabled, deploy applies the promotion target to every status whose strategy is not `RELEASE`, `DISABLEMENT`, or `DELETION`. See [`deploy`](/docs/cli/deploy/) and the [detection-as-code workflow](/docs/usage/workflows/detection-as-code/#status-and-deployment-strategy).

## Deployment plan

The `DEPLOYMENT_PLAN` environment variable selects which plan `deploy` and `validate query --live` operate under, letting you keep separate plans (e.g. per environment) without editing config. The default offline `validate query` grades every rule's query text and ignores the plan:
...
See [CLI global options](/docs/cli/global-options/) for how it resolves.
...
- `documentation.toml` tunes `opentide generate docs` output (which folders, which sections). CVE links and optional API enrichment use [CIRCL Vulnerability-Lookup](https://vulnerability.circl.lu); see [`generate docs`](/docs/cli/generate/).
...
- [Repository setup](/docs/usage/repository-setup/) — how `opentide setup` scaffolds this directory.
- [Troubleshooting](/docs/usage/troubleshooting/) — "unknown status", "platform not loaded", and other config errors.
