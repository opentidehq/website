...
5. CI/CD
   ...
   - Typical job order: pip install 'opentide==0.6.3' → opentide validate → validate query per enabled platform → opentide generate → opentide generate docs --output docs → deploy stages.
