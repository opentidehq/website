...
Optional MCP server setup: `opentide setup mcp --cursor --yes` (after `pip install 'opentide==0.6.2'`).
...
```yaml
- run: pip install 'opentide==0.6.2'
- run: opentide validate --strict
  env:
    OPENTIDE_REPO_ROOT: ${{ github.workspace }}
```

See [CI/CD workflow](/docs/usage/workflows/ci-cd/).
...
Full flag list: [CLI migrate](/docs/cli/migrate/).

## Agent-assisted migration

Replacing a CoreTide **git submodule**, Orchestration scripts, and Python imports is not a single CLI command. Use the [CoreTide migration prompt](/docs/usage/migration/prompt/) with your coding agent.
...
- [ ] `pip install 'opentide==0.6.2'` succeeds
...
- [CoreTide migration prompt](/docs/usage/migration/prompt/)
- [CLI migrate](/docs/cli/migrate/)
