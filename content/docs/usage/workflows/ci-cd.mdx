...
<Callout type="info">
Hand-written examples below pin `pip install 'opentide==0.6.2'` so a pipeline cannot float onto an accidental `0.x.dev…` local build. `opentide setup ci` is the source of truth.
</Callout>
...
steps:
  ...
  - script: pip install 'opentide==0.6.2'
