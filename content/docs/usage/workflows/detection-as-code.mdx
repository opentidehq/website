...
This is the workflow you live in after onboarding: author objects, validate, review, promote, deploy. If you have not built a chain yet, do the [Tutorial](/docs/usage/tutorial/) first — this page assumes you understand the [object model](/docs/usage/concepts/object-model/).
...
- **`metadata.schema`** — the correct revision (`rule::1.0`). See [Schema revision](/docs/usage/concepts/schema-revision/).
...
A rule that references no objective is an [orphan](/docs/usage/concepts/object-model/#anti-patterns-to-avoid). Point `detection_model` at the objective it implements, and make sure that objective references the threat it covers under `objective.threats`. Verify:
...
opentide validates only that a rule's `status` exists in your merged `deployment.toml` — it does not enforce any ordering between statuses, so you can set any configured status directly. If you override `deployment.toml`, these names may not exist at all in your repo. See [Configuration → deployment statuses](/docs/usage/configuration/#deployment-statuses-and-strategies).
...
See [`deploy`](/docs/cli/deploy/) and [Configuration → promotion](/docs/usage/configuration/#promotion).
...
Always run `opentide validate query --platform <name>` for supporting platforms before deploy. opentide reports `supported: false` for CrowdStrike/HarfangLab rather than faking a pass — see [Platforms](/docs/usage/concepts/platforms/).

## Review checklist

Use this in pull-request reviews:

1. Does `validate --strict` pass? (CI enforces it — see [CI/CD](/docs/usage/workflows/ci-cd/).)
...
See [`generate exports`](/docs/cli/generate/).
...
Then see [Agentic setup](/docs/usage/workflows/agentic-setup/) for a safe agent workflow and its limits.
