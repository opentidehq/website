...
`info` shows your objects and how they chain; the coverage query shows that T1059 is now covered by one objective and one rule. See [`info`](/docs/cli/info/).
...
`--dry-run` shows exactly what a real deploy would change without touching the platform. A real deploy needs [credentials](/docs/usage/configuration/#credentials); remove `--dry-run` when you are ready.
