...
Coverage counts rules only; the objective and threat your rule links to are not listed. To see the chain itself, use the Mermaid diagram in [`generate docs`](/docs/cli/generate/) output or the MCP `get_chaining` tool. See [`info`](/docs/cli/info/).
