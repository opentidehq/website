...
`uuid` is the identity that never changes — cross-object links use it, so you can rename or move files freely. `schema` vs `version` trips people up; see [Schema revision](/docs/usage/concepts/schema-revision/).
...
`threat.impact` and `threat.leverage` are non-empty lists of vocabulary names, one name per item. A scalar string or a `;`-joined string is invalid.
...
Cross-object reference and chaining checks run as part of full [validation](/docs/cli/validate/).
...
The same objects are available programmatically through the [registry](/docs/sdk/registry/):
...
Rules loaded through the registry expose delegation methods: `validate()`, `validate_query(platform)`, `deploy()`, and `document()`. See [SDK models](/docs/sdk/models/).
...
- [Schema revision](/docs/usage/concepts/schema-revision/) — `metadata.schema` vs `metadata.version`.
- [Platforms](/docs/usage/concepts/platforms/) — where rules deploy and validate.
- [Tutorial](/docs/usage/tutorial/) — author this exact chain yourself, end to end.
