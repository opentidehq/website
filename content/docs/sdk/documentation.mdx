...
<Callout type="info">
This renders **human wiki pages** from loaded objects. It is distinct from schema/template **generation** (`opentide generate`), which builds framework scaffolding under `.opentide/`. See [How opentide works](/docs/usage/how-it-works/#where-generation-and-deployment-differ).
</Callout>
...
- `render_threat`: criticality/terrain/assessment/actors, CVE (Vulnerability-Lookup links + optional API enrichment), ATT&CK techniques (when present), chaining + chaining details (with backlinks), coverage diagram, related-objects table
...
See [CLI generate](/docs/cli/generate/) for flavor-specific Mermaid behaviour and the CVE section.
